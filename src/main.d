...
class TestScene: Scene
{
    ...
    Wav[2] demonFootsteps;
    Wav[3] demonGrowl;
    Wav demonTauntGrowl;
    Wav demonDamageGrowl;
    Wav demonDyingGrowl;
    float demonGrowlTimer = 0.0f;
    bool demonCanGrowl = true;
    ...
    CharacterPartCollider[7] demonColliders;
    
    Entity eDemon;
    Steering demonSteering;
    
    float demonFootstepTimer = 0.0f;
    uint demonFootstepIndex = 0;
    float demonFootstepFactor = 0.0f;
    
    Entity eParticlesDebris;
    Emitter emitterDebris;
    float shootDebrisTimer = 0.0f;
    
    Entity eParticlesBlood;
    Emitter emitterBlood;
    ...
    override void beforeLoad()
    {
        ...
        demonFootsteps[1] = audio.loadSound("assets/sounds/heavy_footstep2.wav");
        ...
        demonGrowl[0] = audio.loadMusic("assets/sounds/growl1.wav");
        demonGrowl[1] = audio.loadMusic("assets/sounds/growl2.wav");
        demonGrowl[2] = audio.loadMusic("assets/sounds/growl3.wav");
        demonTauntGrowl = audio.loadMusic("assets/sounds/growl4.wav");
        demonDamageGrowl = audio.loadMusic("assets/sounds/growl5.wav");
        demonDyingGrowl = audio.loadMusic("assets/sounds/growl6.wav");
    }
    ...
    override void afterLoad()
    {
        ...
        eDemon = addEntity();
        //eDemon.drawable = aDemon.meshes[0];
        //eDemon.material = aDemon.materials[0];
        //eDemon.material.emissionEnergy = 2.0f;
        eDemon.position = Vector3f(-8.0f, -0.1f, 0.0f);
        eDemon.scaling = Vector3f(3.0f, 3.0f, 3.0f);
        demonSteering = New!Steering(eventManager, eDemon);
        
        useEntity(aDemon.rootEntity, true);
        aDemon.rootEntity.setParent(eDemon);
        
        demonMaterial = aDemon.materials[0];
        
        auto nDemon = aDemon.node("demon");
        demonPose = New!GLTFBlendedPose(nDemon.skin, assetManager);
        auto demonAnimTaunt = aDemon.animation("taunt");
        demonPose.switchToAnimation(demonAnimTaunt, 0.0f, PlayMode.Once);
        nDemon.entity.pose = demonPose;
        
        Entity demonRightArmCollider = addEntity(aDemon.node("right_arm").entity);
        demonRightArmCollider.position = Vector3f(0.0f, 0.1f, -0.05f);
        //demonRightArmCollider.drawable = New!ShapeBox(Vector3f(0.06f, 0.15f, 0.06f), assetManager);
        demonColliders[0] = CharacterPartCollider(demonRightArmCollider, Vector3f(0.06f, 0.15f, 0.06f), 0.25f);
        
        Entity demonLeftArmCollider = addEntity(aDemon.node("left_arm").entity);
        demonLeftArmCollider.position = Vector3f(-0.01f, 0.0f, -0.01f);
        //demonLeftArmCollider.drawable = New!ShapeBox(Vector3f(0.06f, 0.15f, 0.06f), assetManager);
        demonColliders[1] = CharacterPartCollider(demonLeftArmCollider, Vector3f(0.06f, 0.15f, 0.06f), 0.25f);
        
        Entity demonRightForearmCollider = addEntity(aDemon.node("right_forearm").entity);
        demonRightForearmCollider.position = Vector3f(0.0f, 0.17f, -0.05f);
        //demonRightForearmCollider.drawable = New!ShapeBox(Vector3f(0.06f, 0.17f, 0.06f), assetManager);
        demonColliders[2] = CharacterPartCollider(demonRightForearmCollider, Vector3f(0.06f, 0.17f, 0.06f), 0.25f);
        
        Entity demonLeftForearmCollider = addEntity(aDemon.node("left_forearm").entity);
        demonLeftForearmCollider.position = Vector3f(-0.05f, 0.15f, 0.0f);
        //demonLeftForearmCollider.drawable = New!ShapeBox(Vector3f(0.06f, 0.17f, 0.06f), assetManager);
        demonColliders[3] = CharacterPartCollider(demonLeftForearmCollider, Vector3f(0.06f, 0.17f, 0.06f), 0.25f);
        
        Entity demonHeadCollider = addEntity(aDemon.node("head").entity);
        demonHeadCollider.position = Vector3f(-0.04f, 0.1f, 0.02f);
        //demonHeadCollider.drawable = New!ShapeBox(Vector3f(0.1f, 0.1f, 0.1f), assetManager);
        demonColliders[4] = CharacterPartCollider(demonHeadCollider, Vector3f(0.1f, 0.1f, 0.1f), 1.0f);
        
        Entity demonTorsoCollider = addEntity(aDemon.node("spine1").entity);
        demonTorsoCollider.position = Vector3f(-0.04f, 0.17f, 0.0f);
        //demonTorsoCollider.drawable = New!ShapeBox(Vector3f(0.2f, 0.1f, 0.13f), assetManager);
        demonColliders[5] = CharacterPartCollider(demonTorsoCollider, Vector3f(0.2f, 0.1f, 0.13f), 0.75f);
        
        Entity demonSpineCollider = addEntity(aDemon.node("hips").entity);
        demonSpineCollider.position = Vector3f(-0.04f, 0.05f, 0.05f);
        //demonSpineCollider.drawable = New!ShapeBox(Vector3f(0.14f, 0.2f, 0.1f), assetManager);
        demonColliders[6] = CharacterPartCollider(demonSpineCollider, Vector3f(0.14f, 0.2f, 0.1f), 0.5f);
        ...
        auto mParticlesBlood = addMaterial();
        ...
        eParticlesBlood = addEntity();
        emitterBlood = New!Emitter(eParticlesBlood, particleSystem, 10);
        emitterBlood.material = mParticlesBlood;
        emitterBlood.startColor = Color4f(1.0f, 0.0f, 0.0f, 1.0f);
        emitterBlood.endColor = Color4f(1.0f, 0.0f, 0.0f, 0.0f);
        emitterBlood.initialDirectionRandomFactor = 1.0f;
        emitterBlood.initialPositionRandomRadii = Vector3f(0.5f, 0.5f, 0.5f);
        emitterBlood.scaleStep = Vector2f(2.0f, 2.0f);
        emitterBlood.rotationStep = 0.5f;
        emitterBlood.minInitialSpeed = 2.0f;
        emitterBlood.maxInitialSpeed = 5.0f;
        emitterBlood.minSize = 0.5f;
        emitterBlood.maxSize = 1.0f;
        emitterBlood.fadeInDuration = 0.01f;
        emitterBlood.minLifetime = 0.5f;
        emitterBlood.maxLifetime = 1.0f;
        emitterBlood.gravity = Vector3f(0.0f, -5.0f, 0.0f);
        emitterBlood.reset();
        emitterBlood.emitting = false;
        eParticlesBlood.visible = true;
    ...
    override void onUpdate(Time t)
    {
        if (shootRaycast)
        {
            ...
            foreach(ref demonCollider; demonColliders)
            {
                AABB aabb = AABB(Vector3f(0.0f, 0.0f, 0.0f), demonCollider.halfExtents);
                Vector3f rayStartLocal = rayStart * demonCollider.entity.invAbsoluteTransformation;
                Vector3f rayEndLocal = rayEnd * demonCollider.entity.invAbsoluteTransformation;
                if (aabb.intersectsSegment(rayStartLocal, rayEndLocal, intersectionTime))
                {
                    demonHit = true;
                    damageRatio = demonCollider.damageRatio;
                    demonHitPosition = rayStart - fpview.direction * (100.0f * intersectionTime);
                    break;
                }
            }
            ...
            if (demonHit)
            {
                eParticlesBlood.position = demonHitPosition;
                emitterBlood.emitting = true;
                shootDebrisTimer = 1.0f;
                eParticlesBlood.update(t);
                
                if (demonState != 2 && demonState != 3)
                {
                    demonMaterial.emissionTexture = null;
                    demonMaterial.emissionFactor = Color4f(1.0f, 0.0f, 0.0f, 1.0f);
                    demonMaterial.emissionEnergy = 0.5f;
                    
                    demonHealth -= damageRatio;
                
                    if (demonHealth <= 0.0f)
                    {
                        audio.play(demonDyingGrowl);
                        demonHealth = 0.0f;
                        demonState = 2;
                        auto demonAnimDying = aDemon.animation("dying");
                        demonPose.switchToAnimation(demonAnimDying, 0.5f, PlayMode.OnceAndStop);
                        demonPose.timeScale = 1.0f;
                    }
                    else
                    {
                        audio.play(demonDamageGrowl);
                        demonState = 3;
                        auto demonAnimHit = aDemon.animation("hit");
                        demonPose.switchToAnimation(demonAnimHit, 0.05f, PlayMode.Once);
                        demonAnimationSwitchTimer = 0.0f;
                        demonPose.timeScale = 1.0f;
                    }
                }
            }
        ...
        float shakeDistanceFactor = 1.0f - clamp(distance(eDemon.position, eCharacter.position) / 20.0f, 0.0f, 1.0f);
        float shakeFactor = (1.0f - easeOutElastic(1.0f - demonFootstepFactor)) * 0.1f * (shakeDistanceFactor * shakeDistanceFactor);
        ...
        game.postProcessingRenderer.depthOfFieldEnabled = playerAiming;
    ...
    int demonStepFrame1 = 9;
    int demonStepFrame2 = 24;
    
    float demonAnimationSwitchTimer = 0.0f;
    
    float currentAngleY = 0.0f;
    
    void updateDemonAI(Time t)
    {
        Vector3f rayStart = eDemon.position + Vector3f(0.0f, 1.0f, 0.0f);
        Vector3f rayEnd = eDemon.position - Vector3f(0.0f, 100.0f, 0.0f);
        Vector3f hitPosition, hitNormal;
        JoltRigidBody hitBody;
        if (physicsWorld.raycast(rayStart, rayEnd, hitPosition, hitNormal, hitBody))
        {
            demonSteering.positionY += (hitPosition.y - 0.23f - demonSteering.positionY) * 2.0f * t.delta;
        }
        
        if (demonState == 1)
        {
            //eDemon.move(1.17f * t.delta);
            //eDemon.turn(30.0f * t.delta);
            const float animDemonSpeed = 3.5f;
            demonSteering.maxSpeed = animDemonSpeed;
            demonSteering.arriveToDistance(eCharacter.position, 3.0f, 1.0f, 0.1f);
            
            float demonSpeed = demonSteering.velocity.length;
            //float animationSpeedRatio = demonSpeed / animDemonSpeed;
            demonPose.timeScale = 1.0f; //animationSpeedRatio;
            
            float speedFactor = clamp(demonSpeed, 0.0, 10.0f) / 10.0f;
            Vector3f lookDir = lerp((eCharacter.position - eDemon.position).normalized, demonSteering.direction, speedFactor);
            float desiredAngleY = radtodeg(atan2(lookDir.x, lookDir.z));
            float deltaAngle = wrapAngle(desiredAngleY - currentAngleY); // [-180, +180]
            const float maxTurn = 360.0f * t.delta; // 360 degrees/s
            deltaAngle = clamp(deltaAngle, -maxTurn, maxTurn);

            currentAngleY += deltaAngle;
            eDemon.setRotation(0.0f, currentAngleY, 0.0f);
            
            int frame = cast(int)floor(fmod(demonPose.time.elapsed, demonPose.animationDuration) * 24);
            if (frame == demonStepFrame1 && demonFootstepIndex == 0)
            {
                auto voice = audio.playAtPosition(demonFootsteps[demonFootstepIndex], eDemon.position);
                audio.setAttenuation(voice, AttenuationModel.ExponentialDistance, 0.1f);
                demonFootstepIndex = 1;
            }
            if (frame == demonStepFrame2 && demonFootstepIndex == 1)
            {
                auto voice = audio.playAtPosition(demonFootsteps[demonFootstepIndex], eDemon.position);
                audio.setAttenuation(voice, AttenuationModel.ExponentialDistance, 0.1f);
                demonFootstepIndex = 0;
            }
            
            if (distance(eDemon.position, eCharacter.position) <= 3.0f)
            {
                if (demonCanGrowl)
                {
                    audio.playAtPosition(demonGrowl[uniform(0, $)], eDemon.position);
                    demonCanGrowl = false;
                }
                
                demonState = 5;
                demonPose.switchToAnimation(aDemon.animation("punch"), 0.15f, PlayMode.Loop);
                demonPose.timeScale = 1.0f;
            }
        }
        else if (demonState == 3)
        {
            demonSteering.velocity = Vector3f(0.0f, 0.0f, 0.0f);
            demonSteering.acceleration = Vector3f(0.0f, 0.0f, 0.0f);
            demonAnimationSwitchTimer += t.delta;
            if (demonAnimationSwitchTimer > 1.0f)
            {
                int frame = cast(int)floor(fmod(demonPose.time.elapsed, demonPose.animationDuration) * 24);
                if (frame >= 30)
                {
                    demonState = 1;
                    demonPose.switchToAnimation(aDemon.animation("walk1"), 0.15f, PlayMode.Loop);
                    demonFootstepIndex = 0;
                    demonAnimationSwitchTimer = 0.0f;
                    demonPose.timeScale = 1.0f;
                }
        ...
        else if (demonState == 4)
        {
            ...
            if (demonAnimationSwitchTimer > 1.0f)
            {
                ...
                {
                    demonState = 1;
                    demonPose.switchToAnimation(aDemon.animation("walk1"), 0.25f, PlayMode.Loop);
                    demonFootstepIndex = 0;
                    demonAnimationSwitchTimer = 0.0f;
                    demonPose.timeScale = 1.0f;
                }
        ...
        if (demonGrowlTimer < 5.0f && !demonCanGrowl)
        {
            demonGrowlTimer += t.delta;
        }
        else
        {
            demonCanGrowl = true;
            demonGrowlTimer = 0.0f;
        }
        
        if (demonMaterial.emissionEnergy > 0.0f)
        {
            demonMaterial.emissionEnergy -= t.delta;
        }
        else
        {
            demonMaterial.emissionEnergy = 0.0f;
        }
    }
